...
Instances of type java.util.Optional\textless T\textgreater are marshalled to a JSON value as JSON objects when T alone would be marshalled as JSON object. 
When T would be marshalled as a JSON value (e.g. java.lang.String, java.lang.Integer), an instance of java.util.Optional\textless T\textgreater is marshalled as a JSON value (without curly brackets). [JSB-\ref{subsec:optional}-6]

Unmarshalling of a JSON value into java.util.Optional\textless T\textgreater MUST be supported if unmarshalling of a JSON value into instance of T is supported. [JSB-\ref{subsec:optional}-7]
...
JSON object values are unmarshalled into an implementation of java.util.Map \textless String, Object \textgreater with a predictable iteration order.[JSB-\ref{sec:untyped}-2]
...
If a class property is defined with an interface and not concrete type, then the mapping for a marshalled the property is resolved based on its runtime type.[JSB-\ref{sec:interfaces}-3]
